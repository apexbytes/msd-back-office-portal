  ...
  protected loadUsers(): void {
    ...
    this.userService.getAllUsers(params).subscribe({
      next: (res: any) => {
        ...
        // Update pagination signals based on metadata or pagination object
        const pagination = res.pagination || data.pagination || data.metadata;
        if (pagination) {
          this.currentPage.set(pagination.currentPage || 1);
          this.totalPages.set(pagination.totalPages || Math.ceil((pagination.totalCount || 0) / this.limit()) || 1);
          this.totalCount.set(pagination.totalCount || 0);
          this.hasNext.set(pagination.hasNext || this.currentPage() < this.totalPages());
          this.hasPrev.set(pagination.hasPrev || this.currentPage() > 1);
        } else {
